...
class Dinosaur {
    constructor(attribute) {
        ...
        this.newPhrase = attribute.phrase;
    }
    speak() {
...
*/
const animalNames = [];
zooAnimals.forEach(fullNames => {
    let name = `Name: `+fullNames.animal_name + `, ` +`Scientific: ` +fullNames.scientific_name;
...
const lowerCase = [];
let noCaps = zooAnimals.map((name, index, data) => {
    return {
        'name': name.animal_name.toLowerCase()
    };
});
console.log(noCaps);
...
let survive = zooAnimals.filter((pop) => {
...
let allThePopulation = zooAnimals.reduce((accumulator, currentValue) => {
    return accumulator += currentValue.population;
}, 0)
console.log(allThePopulation);
